...
static PTPPropertyMetadata sPropertyMetadata[] = {
    ...
    META_ENUM_U8 ("auto-focus-adjust-enabled", DPC_AUTO_FOCUS_ADJUST_ENABLED, sProp_EnabledDisabled),
...
static PtpPropNames sPtpPropertyLabels[] = {
    ...
    {DPC_IRIS_CLOSE_MODE, "Iris Close Mode"},
    ...
    {DPC_SHUTTER_ELECTRONIC_MODE, "Electronic Shutter Mode"},
    ...
    {0xD02A, "FTP Sync ID"},
    {0xD02B, "Media Slot 1 Rec Available Type"},
    {0xD02C, "Media Slot 2 Rec Available Type"},
    ...
    {0xD043, "Rec Settings Reset Enabled"},
    ...
    {0xD1D4, "Media Slot 1 ContentInfoList Enabled"},
    {0xD1D5, "Media Slot 2 ContentInfoList Enabled"},
    {0xD1D6, "Media Slot 1 ContentInfoList Regenerate Time"},
    {0xD1D7, "Media Slot 2 ContentInfoList Regenerate Time"},
    ...
    {DPC_AUTO_FOCUS_ADJUST_ENABLED, "Auto Focus Adjust Enabled"},
    ...
    {0xD26D, "Custom WB Capture Standby"},
    {0xD26E, "Custom WB Capture Standby Cancel"},
    {0xD26F, "Custom WB Capture Operation"},
    {0xD270, "Custom WB Execution State"},
    {DPC_CAMERA_SETTING_SAVE_ENABLED, "Camera Settings Save"},
    {DPC_CAMERA_SETTING_READ_ENABLED, "Camera Settings Read"},
    {DPC_CAMERA_SETTING_SAVE_READ_STATE, "Camera Settings Save/Read State"},
    {0xD274, "FTP Settings Save"},
    {0xD275, "FTP Settings Read"},
    {0xD276, "FTP Settings Save/Read State"},
    {0xD278, "Live View URL"},
    {DPC_FORMAT_MEDIA_SLOT1_ENABLED, "Media Slot 1 Format Enabled"},
    {DPC_FORMAT_MEDIA_SLOT2_ENABLED, "Media Slot 2 Format Enabled"},
    ...
    {DPC_FORMAT_MEDIA_QUICK_SLOT1_ENABLED, "Media Slot 1 Format Quick Enabled"},
    {DPC_FORMAT_MEDIA_QUICK_SLOT2_ENABLED, "Media Slot 2 Format Quick Enabled"},
    {DPC_FORMAT_MEDIA_CANCEL_ENABLED, "Media Format Cancel Enabled"},
    ...
    {0xE0CE, "Live View Image Quality by Numerical"},
    {0xE0D2, "Red Tally Lamp Control"},
    {0xE0D3, "Green Tally Lamp Control"},
    {0xE0D4, "Yellow Tally Lamp Control"},
    {0xE0D5, "Debug Mode"},
    {0xE0D9, "Media Slot 1 ContentInfoList Update Time"},
    {0xE0DA, "Media Slot 2 ContentInfoList Update Time"},
    {0xE0E1, "Call Setting"},
    ...
    {0xE0F3, "Media Slot 1 DeleteContent Enabled"},
    {0xE0F4, "Media Slot 2 DeleteContent Enabled"},
...
#define PROP_ENUM_SET(a) .form.enums = {.values.data=(a), .size=MStaticArraySize(a)}
...
static void PrintProperties(AwControl* self) {
    ...
    for (int i = 0; i < numProperties; i++) {
        AwPtpProperty* p = self->properties.data + i;
            ...
            case PTP_FORM_FLAG_ENUM: {
                    ...
                    if (numSetValues > 0) {
                        ...
                        for (int j = 0; j < numSetValues; j++) {
                            PrintPropertyValue(p->dataType, p->form.enums.set.data + j);
                        }
                    }
                    ...
                    if (numGetSetValues > 0) {
                        ...
                        for (int j = 0; j < numGetSetValues; j++) {
                            PrintPropertyValue(p->dataType, p->form.enums.getSet.data + j);
                        }
                    }
                }
                break;
    ...
    for (int i = 0; i < numProperties; i++) {
        ...
        if (AwControl_SupportsControl(self, propCode)) {
            ...
            if (control->formFlag == PTP_FORM_FLAG_ENUM) {
                ...
                MArrayInitZ(self->allocator, control->form.enums.values, numEnumSet);
...
static void SDIO_ProcessDeviceProperties300(AwControl *self, b32 initial, PTPResponse r, u64 numProperties) {
    if (initial) {
        MArrayInitZ(self->allocator, self->properties, numProperties);
    }
...
static AwResult PTP_GetLiveViewImage(AwControl* self, size_t objectSize, MMemIO* fileOut, AwLiveViewFrames* liveViewFrames) {
    ...
    if (readFocalFrame) {
        ...
        if (focalFrameSize) {
            ...
            } else if (!MArrayIsEmpty(focusFrames->frames)) {
                MArrayClear(focusFrames->frames);
            }
...
AwResult AwControl_ReadEvents(AwControl* self, int timeoutMilliseconds, MAllocator* alloc, AwPtpEventArray* eventsOut) {
...
AwResult AwControl_GetMagnifier(AwControl* self, AwMagnifier* outMagnifier) {
    ...
    if (propMagPos) {
        ...
        if (propMag) {
            ...
            AwPtpPropValue* enums = propMag->form.enums.getSet.data;
            size_t enumCount = MArraySize(propMag->form.enums.getSet);
            if (!enums) {
                enums = propMag->form.enums.set.data;
                enumCount = MArraySize(propMag->form.enums.set);
            }

            outMagnifier->numRatios = enumCount;
...
static void SDIO_InitControlsMetadata200(AwControl *self, size_t numControls) {
    for (int i = 0; i < numControls; i++) {
        u16 controlCode = self->supportedControls.data[i];
...
static void SDIO_InitControlsMetadata300(AwControl *self, size_t numControls) {
    ...
    for (int i = 0; i < numControls; i++) {
        ...
        if (!found) {
            ...
            control->form.enums.values.data = sControl_UpDown;
...
b32 BuildEnumsFromGetFunc(AwControl* self, MAllocator* allocator, AwPtpProperty* property, PTPPropertyMetadata* meta,
    ...
    for (int i = 0; i < MArraySize(property->form.enums.getSet); i++) {
        AwPtpPropValueEnum *propEnum = MArrayAddPtr(allocator, outEnums->values);
        AwPtpPropValue propValue = property->form.enums.getSet.data[i];
    ...
    if (MArraySize(property->form.enums.set)) {
        ...
        for (int i = 0; i < MArraySize(property->form.enums.set); i++) {
            AwPtpPropValue lookupValue = property->form.enums.set.data[i];
            AwPtpPropValueEnum* prop = NULL;
            for (int j = 0; j < getSetItems; j++) {
...
AW_EXPORT AwResult AwControl_SetPropertyBool(AwControl* self, AwPtpProperty* property, b32 value) {
    ...
    if (meta->fixedEnumsSize) {
        ...
        switch (meta->type) {
            case PTP_DT_UINT8: {
                for (int i = 0; i < MArraySize(property->form.enums.getSet); i++) {
                    u8 val = property->form.enums.getSet.data[i].u8;
            ...
            case PTP_DT_UINT16: {
                for (int i = 0; i < MArraySize(property->form.enums.getSet); i++) {
                    u16 val = property->form.enums.getSet.data[i].u16;
                    for (int j = 0; j < meta->fixedEnumsSize; j++) {
            ...
            case PTP_DT_UINT32: {
                for (int i = 0; i < MArraySize(property->form.enums.getSet); i++) {
                    u32 val = property->form.enums.getSet.data[i].u32;
                    for (int j = 0; j < meta->fixedEnumsSize; j++) {
...
AwPtpControl* AwControl_GetControlByIndex(AwControl* self, u16 index) {
    ...
    return self->controls.data + index;
}
